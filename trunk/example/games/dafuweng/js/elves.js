(function(window, undefined) {
	...
	window.Elves = window.$ = (function() {
		var Elves = function(selector) {
			if ( typeof selector === "string") {
				if (selector.charAt(0) === "$") {
					if (selector.indexOf('@') >= 0) {
						var s = selector.split('@');
						return new Elves.fn.Entity(s[0], s[1]);
					}
					return new Elves.fn.Canvas(selector);
				} else {
					return new Elves.fn.Dom(selector);
				}
			}

			if (Elves.isFunction(selector)) {
				window.addEventListener('load', selector, false);
				return this;
			}
		};

		Elves.fn = Elves.prototype = {
			constructor : Elves,
			...
			Animation : function(selector) {
				if ( typeof selector === 'string') {
					...
					for (var field in selector ) {
						if (Elves.isNumeric(selector[field])) {
		...
		Elves.fn.Dom.fn = Elves.fn.Dom.prototype;
		Elves.fn.Canvas.fn = Elves.fn.Canvas.prototype;
		Elves.fn.Entity.fn = Elves.fn.Entity.prototype;
		Elves.fn.Animation.fn = Elves.fn.Animation.prototype;

		Elves.extend = Elves.fn.Dom.fn.extend = Elves.fn.Canvas.fn.extend = Elves.fn.Entity.fn.extend = Elves.fn.Animation.fn.extend = function() {
			...
			if ( typeof target !== "object" && !Elves.isFunction(target)) {
				target = {};
			}

			// extend Elves itself if only one argument is passed
			if (length === i) {
				target = this;
				--i;
			}
			...
			for (; i < length; i++) {
				...
				if (( options = arguments[i]) != null) {
					...
					for (name in options ) {
						...
						if (deep && copy && (Elves.isPlainObject(copy) || ( copyIsArray = Elves.isArray(copy)) )) {
							if (copyIsArray) {
								copyIsArray = false;
								clone = src && Elves.isArray(src) ? src : [];
							} else {
								clone = src && Elves.isPlainObject(src) ? src : {};
							}

							// Never move original objects, clone them
							target[name] = Elves.extend(deep, clone, copy);
		...
		Elves.extend({
			isFunction : function(obj) {
				return Elves.type(obj) === "function";
			},
			...
			isPlainObject : function(obj) {
				...
				// - DOM nodes
				// - window
				if (Elves.type(obj) !== "object" || obj.nodeType || Elves.isWindow(obj)) {
					return false;
				}
			...
			makeArray : function(arr, results) {
				...
				if (arr != null) {
					if (isArraylike(Object(arr))) {
						Elves.merge(ret, typeof arr === "string" ? [arr] : arr);
					} else {
		...
		Elves.each("Boolean Number String Function Array Date RegExp Object Error".split(" "), function(i, name) {
			class2type["[object " + name + "]"] = name.toLowerCase();
		});

		function isArraylike(obj) {
			var length = obj.length, type = Elves.type(obj);

			if (Elves.isWindow(obj)) {
				return false;
			}
		...
		Elves.fn.Dom.fn.extend({
			canvas : function(id) {
				...
				return new Elves.fn.Canvas('$' + id);
			}
		});

		Elves.fn.Canvas.fn.extend({
			entity : function(id, option) {
				...
				if (id && typeof id === 'string') {
					for (var i = 0; i < entries.length; i++) {
						if (entries[i].id === id) {
							if (option) {
								entries[i] = Elves.extend(entries[i], option);
							}
							return new Elves.fn.Entity(this.id, id);
						}
					}
					...
					if (option) {
						entity = Elves.extend(entity, option);
					}
					entries.push(entity);
			...
			clear : function(x, y) {
				var cx = x || this[0].width, cy = y || this[0].height;
				this.context.clearRect(0, 0, cx, cy);
				return this;
			},
			...
			update : function() {
				if (this[0].autoAnimation) {
					...
					for (var i = 0; entries && i < entries.length; i++) {
						new Elves.fn.Entity(this.id, entries[i].id).draw();
					}
				}
				return this;
			}
		...
		Elves.fn.Entity.fn.extend({
			draw : function() {
				...
				if ('draw' in option && Elves.isFunction(option.draw)) {//可以自带flush方法
			...
			animation : function() {
				return new Elves.fn.Animation(this[0]);
			},
			hide : function() {
				this[0]._hide = true;
				if(TWEEN.size() === 0){
					Elves(this[1].id).update();
				}
				return this;
			},
			show : function() {
			...
			bind : function(eventTag, fn, isPropagation) {
				var events = onfireListeners[eventTag] || [], isPropagation = isPropagation || true;
				events.push({
					data : this[0],
					canvas_id : this[1].id,
					fn : fn,
	...
	var _touchstart = (Elves.isAndroid() ? 'touchstart' : 'mousedown');
	window.addEventListener(_touchstart, function(event) {
		var target = Elves.isAndroid() ? event.changedTouches[0] : event, x = target.pageX, y = target.pageY, listeners = onfireListeners['touchstart'];
		//console.log(listeners);
		if (listeners) {
			for (var i = 0; i < listeners.length; i++) {
				var e = listeners[i], shape = e.data.shape || 'rect', w = e.data.dw || e.data.w || 0, h = e.data.dh || e.data.h || 0, dx = e.data.translateX ? e.data.translateX + e.data.dx : e.data.dx, dy = e.data.translateY ? e.data.translateY + e.data.dy : e.data.dy,canvas = canvas_cache[e.canvas_id],tx=x-canvas.left,ty=y-canvas.top;
				if (shape === 'rect') {
					//console.log("run:x >"+x+"| dx >"+dx+"| y>"+y+"| dy>"+dy+"| h>"+h+"| w>"+w);
					if (tx > dx && ty > dy && tx < dx + w && ty < dy + h) {
						e.fn.call(e.data, tx, ty);
						if (!e.isPropagation) {
							break;
						}
					}
				} else if (shape === 'circle') {
					var r = w / 2, cx = dx + r, cy = dy + r, dist = Math.sqrt(Math.pow(Math.abs(cx - tx), 2) + Math.pow(Math.abs(cy - ty)));
					if (dist < r) {
						e.fn.call(e.data, tx, ty);
						if (!e.isPropagation) {
							break;
